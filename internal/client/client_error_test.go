...
func TestClient_UnparsableError(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte("Critical System Failure"))
	}))
	defer server.Close()
...
func TestClient_EmptyBodyError(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "empty response body") {
		...
		// If unmarshalResourceResponse is called.
		// Wait, doRequest returns bodyBytes. 
		// If body is empty, doRequest returns empty bytes.
		// Then GetResource -> GetResourceWithModule -> json.Unmarshal(body, dest)
		// json.Unmarshal of empty bytes is an error: "unexpected end of JSON input"
		// UNLESS doRequest handles it? doRequest doesn't check empty body for success codes.
		// unmarshalResourceResponse DOES check: if len(body) == 0 return fmt.Errorf("empty response body")
		// BUT GetResourceWithModule calls json.Unmarshal DIRECTLY.
...
func TestClient_CreateResource_ResourceChanges(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet {
			json.NewEncoder(w).Encode(ResourceResponse[map[string]any]{
				Name: "new-resource",
				Signature: "sig-789",
			})
		}
